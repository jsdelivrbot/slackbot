...
module.exports = function(webserver, controller) {
            ...
            slackapi.api.oauth.access(opts, function(err, auth) {
                ...
                slackapi.api.auth.test({token: auth.access_token}, function(err, identity) {
                    ...
                    if (err) {
                        ...
                        return res.redirect('/oauth_error');
                    }
                    ...
                    req.session.team_id = auth.team_id;
                    req.session.bot_user_id = auth.bot.bot_user_id;
                    res.redirect('/');
    ...
    webserver.get('/oauth_error', function(req, res){
      res.render('oauth_error', {
        domain: req.get('host'),
